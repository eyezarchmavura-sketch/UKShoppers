...
const initialForm = { storeName: "", title: "", details: "", sourceType: "official_retailer" as const, sourceUrl: "", termsSummary: "", linkType: "direct" as const, offerUrl: "", couponCode: "", validUntil: "", status: "draft" as OfferStatus };
...
export default function SeasonalOffersAdmin() {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    create.mutate({
      ...
      sourceType: form.sourceType, sourceUrl: form.sourceUrl.trim() || null, termsSummary: form.termsSummary.trim() || null, linkType: form.linkType,
  ...
  const setStatus = (offer: ManagedOffer, status: OfferStatus) => update.mutate({
    ...
    offer: {
      ...
      sourceType: offer.sourceType, sourceUrl: offer.sourceUrl, termsSummary: offer.termsSummary, linkType: offer.linkType,
  ...
  return <main className="min-h-screen bg-[#F2F4F7] py-7"><div className="container max-w-6xl space-y-7"><header className="flex flex-col justify-between gap-4 rounded-3xl bg-[#111418] p-6 text-white shadow-lg sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#D4AF37]">Operations content control</p><h1 className="mt-1 text-2xl font-bold">Verified seasonal offers</h1><p className="mt-2 max-w-2xl text-sm text-white/70">Publish only promotions confirmed from the retailer’s official page. No estimates, expired offers, or unverified codes.</p></div><Link href="/admin" className="rounded-full bg-white/10 px-4 py-2 text-sm font-semibold hover:bg-white/20">Back to queue</Link></header>
    ...
    <section className="grid gap-7 lg:grid-cols-[0.92fr_1.08fr]"><form onSubmit={submit} className="rounded-3xl border border-border bg-white p-6 shadow-sm"><div className="flex items-center gap-3"><div className="rounded-xl bg-[#C9A227]/15 p-2 text-[#A67C00]"><Plus className="h-5 w-5" /></div><div><h2 className="font-bold text-[#111418]">Add verified offer</h2><p className="text-xs text-muted-foreground">Drafts remain private. Publication requires a source, terms and confirmed end date.</p></div></div><div className="mt-6 space-y-4"><label className="block text-sm font-semibold">UK retailer<input required value={form.storeName} onChange={(event) => setForm({ ...form, storeName: event.target.value })} placeholder="e.g. ASOS" className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]" /></label><label className="block text-sm font-semibold">Offer headline<input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Use the retailer's own promotion title" className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]" /></label><label className="block text-sm font-semibold">Verified details<textarea required value={form.details} onChange={(event) => setForm({ ...form, details: event.target.value })} placeholder="State the terms customers need to know; do not add claims you cannot verify." className="mt-1.5 min-h-28 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-semibold">Evidence source<select value={form.sourceType} onChange={(event) => setForm({ ...form, sourceType: event.target.value as typeof form.sourceType })} className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]"><option value="official_retailer">Official retailer</option><option value="approved_partner">Approved partner feed</option><option value="manual_verification">Manual verification</option></select></label><label className="block text-sm font-semibold">Evidence URL<input type="url" required={form.status === "published"} value={form.sourceUrl} onChange={(event) => setForm({ ...form, sourceUrl: event.target.value })} placeholder="Official source or partner record" className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]" /></label></div><label className="block text-sm font-semibold">Customer-facing terms summary<textarea required={form.status === "published"} value={form.termsSummary} onChange={(event) => setForm({ ...form, termsSummary: event.target.value })} placeholder="Eligibility, exclusions and any stock or end-date conditions." className="mt-1.5 min-h-20 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-semibold">Customer destination URL<input type="url" required={form.status === "published"} value={form.offerUrl} onChange={(event) => setForm({ ...form, offerUrl: event.target.value })} placeholder="https://…" className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]" /></label><label className="block text-sm font-semibold">Link disclosure<select value={form.linkType} onChange={(event) => setForm({ ...form, linkType: event.target.value as typeof form.linkType })} className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]"><option value="direct">Direct retailer link</option><option value="affiliate">Partner / affiliate link</option></select></label></div><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-semibold">Public code, if verified<input value={form.couponCode} onChange={(event) => setForm({ ...form, couponCode: event.target.value })} placeholder="Optional" className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]" /></label><label className="block text-sm font-semibold">Valid until<input required={form.status === "published"} type="date" value={form.validUntil} onChange={(event) => setForm({ ...form, validUntil: event.target.value })} className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]" /></label></div><label className="block text-sm font-semibold">Visibility<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as OfferStatus })} className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#C9A227]"><option value="draft">Draft — private</option><option value="published">Published — public</option><option value="expired">Expired — private</option></select></label></div><Button type="submit" disabled={create.isPending} className="mt-6 w-full bg-[#111418] text-[#D4AF37] hover:bg-black">{create.isPending ? "Saving…" : "Save verified offer"}</Button></form>

      <div className="space-y-5"><section className="rounded-3xl border border-[#C9A227]/35 bg-[#FFFCF0] p-6 shadow-sm"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2 text-[#8A6500]"><CheckCircle2 className="h-4 w-4" /><p className="text-[11px] font-bold uppercase tracking-[0.13em]">Daily publication gate</p></div><h2 className="mt-2 font-bold text-[#111418]">Women-first Verified Store Desk</h2><p className="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">Check official retailer sources first. A draft stays private until the source, terms, future expiry, and customer destination have all been checked again.</p></div><div className="inline-flex items-center gap-2 self-start rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-[#7D5A00] ring-1 ring-[#C9A227]/25"><Clock3 className="h-3.5 w-3.5" />Six or fewer live cards</div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-white/80 p-4"><p className="text-xs font-bold text-[#111418]">Morning priority</p><p className="mt-1 text-xs leading-5 text-muted-foreground">ASOS, LOOKFANTASTIC, Boots, and Superdrug. Review Zara, Next, and M&amp;S three times each week.</p></div><div className="rounded-2xl bg-white/80 p-4"><p className="text-xs font-bold text-[#111418]">Remove, don’t guess</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Unpublish immediately if a source changes, an offer expires, terms are unclear, or a customer reports a mismatch.</p></div></div><div className="mt-4 grid gap-2 text-xs leading-5 text-[#31363B]"><p>1. Record the official UK source and exact customer terms.</p><p>2. Set a verified future expiry and check the customer destination.</p><p>3. Re-open the source immediately before publishing.</p><p>4. Use an affiliate disclosure whenever a tracked partner link is used.</p></div></section><section className="rounded-3xl border border-border bg-white p-6 shadow-sm"><div className="flex items-center justify-between gap-4"><div><h2 className="font-bold text-[#111418]">Offer register</h2><p className="mt-1 text-xs text-muted-foreground">Only published offers with source evidence, customer terms and a future end date reach the hero section.</p></div><div className="rounded-full bg-[#C9A227]/15 px-3 py-1 text-xs font-bold text-[#7D5A00]">{offers.length} saved</div></div>{offersQuery.isLoading ? <div className="py-16 text-center text-sm text-muted-foreground">Loading offer register…</div> : offers.length === 0 ? <div className="py-16 text-center"><Sparkles className="mx-auto h-8 w-8 text-[#A67C00]" /><h3 className="mt-3 font-bold text-[#111418]">No offers saved yet</h3><p className="mt-1 text-sm text-muted-foreground">The public site will show an honest empty state until a verified offer is published.</p></div> : <div className="mt-5 space-y-3">{offers.map((offer) => <article key={offer.id} className="rounded-2xl border border-border p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#A67C00]">{offer.storeName}</span><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${offer.status === "published" ? "bg-emerald-100 text-emerald-800" : offer.status === "expired" ? "bg-slate-100 text-slate-700" : "bg-amber-100 text-amber-900"}`}>{offer.status}</span></div><h3 className="mt-1 font-bold text-[#111418]">{offer.title}</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">{offer.details}</p><p className="mt-2 text-[11px] text-muted-foreground">Source: {offer.sourceType ? offer.sourceType.replace(/_/g, " ") : "not supplied"} · verified {offer.verifiedAt ? displayDate(offer.verifiedAt) : "not yet"}</p><p className="mt-1 text-[11px] text-muted-foreground">Valid until: {displayDate(offer.validUntil)}</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={update.isPending} onClick={() => setStatus(offer, offer.status === "published" ? "draft" : "published")} className="rounded-full border border-border px-3 py-1.5 text-xs font-bold text-[#111418] hover:border-[#C9A227]">{offer.status === "published" ? "Unpublish" : "Publish"}</button><button type="button" disabled={update.isPending} onClick={() => setStatus(offer, "expired")} className="rounded-full border border-border px-3 py-1.5 text-xs font-bold text-[#111418] hover:border-[#C9A227]">Expire</button>{user?.role === "admin" && <button type="button" aria-label={`Delete ${offer.title}`} disabled={remove.isPending} onClick={() => remove.mutate({ id: offer.id })} className="rounded-full border border-red-200 p-1.5 text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>}</div></div></article>)}</div>}</section></div></section>
  </div></main>;
}
