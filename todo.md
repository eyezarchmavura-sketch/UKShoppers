...
# TASK — Approved affiliate deal-feed provider selection 2026-08-27
- [x] Research official affiliate-network APIs and product/promotion feeds for UK retail coverage, confirming access model, retailer approvals, permitted fields and non-scraping compliance from provider documentation. Reviewed and cited Awin, Rakuten Advertising and impact.com documentation; no retailer pages were scraped.
- [x] Compare the evidence-backed provider options for UK Shoppers Africa’s women-first retailer priorities and present a recommendation without claiming retailer availability before partner-account approval. Awin is recommended as the first application based on documented LOOKFANTASTIC UK and M&S UK programme/feed evidence; the full comparison is in `docs/AFFILIATE_DATA_PROVIDER_RECOMMENDATION_2026-08-27.md`.
- [ ] Obtain the owner’s selected network, authorised retailers, content permissions and secure access configuration before building any external data ingestion or creating a schedule.
- [ ] Implement and validate provider-specific private candidate ingestion only after commercial approval and secure credentials are configured; retain staff approval as the default publication gate.
